  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        <Link href="/" className="flex items-center space-x-2 mr-6" onClick={() => setMobileMenuOpen(false)}>
          <ShieldCheck className="h-6 w-6 text-blue-600" />
          <span className="font-bold text-xl inline-block text-slate-900">
          ...
          {session ? (
            <div className="flex items-center space-x-4 border-l pl-4 border-slate-200">
              {session.subscriptionStatus === 'active' ? (
                <span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">PREMIUM</span>
              ) : (
                <Link href="/pricing" className="text-sm font-bold text-blue-600 hover:text-blue-700 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-100 transition-colors">Go Premium</Link>
              )}
              <span className="text-sm font-medium text-slate-700">Hi, {session.name || 'User'}</span>
              <LogoutButton />
            </div>
          ) : (
